...
//   }
  ...
  auto queue = m_acc_env->newQueue();
  {
    ...
    ENUMERATE_ENV(ienv,mm) {
      ...
      EnvCellVectorView envcells = env->envView();
      command << RUNCOMMAND_MAT_ENUMERATE(EnvAndGlobalCell, evgi, envcells) {
	auto [evi,cid] = evgi();
	Int32 index = evgi.index();
        ...
        // masses matériels (partiels)
        out_u_lagrange[cid][nb_total_env + index_env] = in_cell_volume[evi] * in_density[evi];
        // energies matériels (partiels)
        out_u_lagrange[cid][2 * nb_total_env + index_env] = in_cell_volume[evi] * in_density[evi] * in_internal_energy[evi];
        // Quantites de mouvement centrees
        ...
        // Pseudo partiel pour la quantité de mouvement
        out_u_lagrange[cid][3 * nb_total_env + 4] = in_cell_volume[evi] * in_pseudo_viscosity[evi];
...
void MahycoModule::remap() {
  ...
  auto out_pseudo_viscosity_n = ax::viewOut(command, m_pseudo_viscosity_n);
  ...
  auto out_tau_density        = ax::viewOut(command, m_tau_density);
