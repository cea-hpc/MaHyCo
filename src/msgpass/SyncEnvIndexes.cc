...
SyncEnvIndexes::SyncEnvIndexes(MatVarSpace mvs, IMeshMaterialMng* mm,
  ...
  m_buf_owned_mvi        (platform::getAcceleratorHostMemoryAllocator()),
  m_indexes_owned_mvi_pn (platform::getAcceleratorHostMemoryAllocator()),
  m_nb_owned_mvi_pn      (platform::getAcceleratorHostMemoryAllocator()),
  
  m_buf_ghost_mvi        (platform::getAcceleratorHostMemoryAllocator()),
  m_indexes_ghost_mvi_pn (platform::getAcceleratorHostMemoryAllocator()),
  m_nb_ghost_mvi_pn      (platform::getAcceleratorHostMemoryAllocator()),
...
void SyncEnvIndexes::updateEnvIndexes() 
{
...

  {
    ...
    m_indexes_owned_mvi_pn.resize(m_nb_nei);
    m_indexes_ghost_mvi_pn.resize(m_nb_nei);
    m_nb_owned_mvi_pn.resize(m_nb_nei);
    m_nb_ghost_mvi_pn.resize(m_nb_nei);

    Integer accu_nb_owned=0;
    Integer accu_nb_ghost=0;
    for(Integer inei=0 ; inei<m_nb_nei ; ++inei) {
      m_nb_owned_mvi_pn[inei] = m_mmvs->sharedItems(inei).size();
      m_nb_ghost_mvi_pn[inei] = m_mmvs->ghostItems(inei).size();

      m_indexes_owned_mvi_pn[inei] = accu_nb_owned;
      m_indexes_ghost_mvi_pn[inei] = accu_nb_ghost;
      
      accu_nb_owned += m_nb_owned_mvi_pn[inei];
      accu_nb_ghost += m_nb_ghost_mvi_pn[inei];
    }
    m_buf_owned_mvi.resize(accu_nb_owned);
    m_buf_ghost_mvi.resize(accu_nb_ghost);

    // On construit des multi-vues sur des zones allouées en mémoire managées
    MultiArray2View<MatVarIndex> owned_mvi_pn(m_buf_owned_mvi.view(),
    ...
    for(Integer inei=0 ; inei<m_nb_nei ; ++inei) {
      mvi2mvi(m_mmvs->sharedItems(inei), owned_mvi_pn[inei]);
      mvi2mvi(m_mmvs->ghostItems(inei) , ghost_mvi_pn[inei]);
    }

    // Puisque les adresses ont potentiellement changé : 
    // "Conseils" mémoire
    if (m_acc_mem_adv) {
      m_acc_mem_adv->setReadMostly(m_buf_owned_mvi       .view());
      m_acc_mem_adv->setReadMostly(m_indexes_owned_mvi_pn.view());
      m_acc_mem_adv->setReadMostly(m_nb_owned_mvi_pn     .view());
      m_acc_mem_adv->setReadMostly(m_buf_ghost_mvi       .view());
      m_acc_mem_adv->setReadMostly(m_indexes_ghost_mvi_pn.view());
      m_acc_mem_adv->setReadMostly(m_nb_ghost_mvi_pn     .view());
    }
  }

  if (m_evlist_per_env_needed)
  {
