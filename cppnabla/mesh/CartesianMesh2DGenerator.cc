...
CartesianMesh2DGenerator::generate(int nbXQuads, int nbYQuads, double xSize, double ySize)
{
	...
	{
		if (j==0 && i!=0 && i!=nbXQuads) bottom_node_ids_[bottom_node_id_++] = node_id_;
		if (j==nbYQuads && i!=0 && i!=nbXQuads) top_node_ids_[top_node_id_++] = node_id_;
		if (i==0 && j!=0 && j!=nbYQuads) left_node_ids_[left_node_id_++] = node_id_;
		if (i==nbXQuads && j!=0 && j!=nbYQuads) right_node_ids_[right_node_id_++] = node_id_;
